...
(function(jQuery, window, document){
	...
	jQuery.fn.responsiveSwipe = function (useropts) {
		var
			...
			throttle,
			cache = {},
			canonicalLink = jQuery('link[rel=canonical]'),
			clickType = 'initial',
			contentArea = jQuery(this)[0],
			contentAreaTop = jQuery(this).offset().top,
			editionPos = -1,
			edition = [],
			editionLen = 0,
			editionChecksum,
			...
			ajaxCount = 0,
			noHistoryPush,
			paneVisible = jQuery(this).find('#swipeview-slider > #swipeview-masterpage-1')[0],
			pageData,
			panes,
			paneNow = 1,
			paneThen = 1,
		...
		var getPageData = function () {
			try {
				var obj = jQuery(paneVisible).find(opts.pageDataSelector).eq(0).data('pagedata');
		...
		var populate = function (el, html, cssClass) {
			...
			jQueryel.html(html);
			var innerEl = jQueryel.find('.pageBodyInner');
		...
		var updateHeight = function(){
			var height = jQuery(paneVisible).children().height();
			if (height) {
				jQuery(contentArea).height(height + paneVisibleMargin + 30);
			}
		};
		...
		var doAfterShow = function () {
			...
			if (initialPage) {
				...
				if (supportsHistory) {
					cache[initialPage] = jQuery(paneVisible).html();
				}
		...
		var spinner = (function () {
			var
				el = jQuery(opts.loadingIndicator),
				obj = {};
			if(el.length) {
				obj.show = function () {
					el.show();
		...
		var posInEdition = function (url) {
			return jQuery.inArray(normalizeUrl(url), edition);
		};
		...
		var setEdition = function (arr) {
			...
			if (jQuery.isArray(arr) && arr.length >= 3) {
		...
		var appSetup = function () {
			...
			// Bind clicks
			jQuery(document).on('click', opts.linkSelector, function (e) {
				var
					url;
				if (!validateClick(e)) { return true; }
				e.preventDefault();
				url = normalizeUrl(jQuery(this).attr('href'));
		...
		var appSetupSwipe = function () {
			...
			var preparePane = function (o) {
				...
				if (el.dataset.url !== url || jQuery.isEmptyObject(cache)) {
			...
			jQuery(window).scroll(function(){
				paneHiddenMargin = Math.max( 0, jQuery(window).scrollTop() - contentAreaTop );
				if( paneHiddenMargin < paneVisibleMargin ) {
					// We've scrolled up over the offset; reset all margins and jump to topmost scroll
					jQuery(panes.masterPages).css('marginTop', 0 );
					jQuery(window).scrollTop( contentAreaTop );
					paneVisibleMargin = 0;
					paneHiddenMargin = 0;
				}
				else {
					// We've scrolled down; push sidepanes down to level of current pane
					jQuery(panes.masterPages).not(':eq(' + paneNow + ')').css('marginTop', paneHiddenMargin );
				}
			});
			...
			panes.onFlip(function () {
				...
				if (paneThen !== paneNow) {
					...
					jQuery(panes.masterPages[paneThen]).css('marginTop', paneHiddenMargin);
			...
			jQuery('body').addClass('has-swipe');
		};
